  ...
  return (
    <div
      ...
      <div className="p-5  flex flex-col rounded-lg">
        <h1
          className={` z-99  text-5xl lg:text-8xl p-3 font-bold flex gap-3 justify-center items-center ${isDark ? "animate-pulse" : "z-2"}`}
          style={{ textShadow: isDark ? "0 0 25px" : "" }}>
          <span
            className={` ${isDark ? "text-cyan-400" : " text-cyan-700"} flex gap-2`}>
            <FaReact
              className={`${isDark ? "text-cyan-400  drop-shadow-[0_0_25px_#22d3ee]" : " text-cyan-700 "}`}
            />
            React
          </span>{" "}
          Portfolio
        </h1>
        <div className="mb-5 z-2">
          <p
        ...
        <div
          className={`flex items-center justify-center ${!singlePage ? "hidden" : "z-2"}`}>
      ...
      <div
        className={`absolute bg-slate-700/10 min-h-screen w-full flex items-center justify-center ${isDark ? "opacity-10 " : "opacity-25 z-[-1]"}text-9xl text-cyan-500 `}>
        <FaReact size={500} />
      </div>
    </div>
  );
