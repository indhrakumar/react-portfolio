...
const ProjectCard = ({ title, image, repolink, openLink, techIcon, about }) => {
  ...
  return (
    <div className="pb-3 h-auto bg-bgCard rounded-lg text-2xl">
      <h1 className="text-center mt-2">{title}</h1>
      <div className="w-full px-1 rounded-t-2xl h-60 mt-4 overflow-hidden">
        <img src={image} alt="" className="w-full h-full object-cover" />
      </div>
      <div className="px-3 py-2">
        <p className="h-35 text-xl overflow-scroll ">
          <span
            className={`block text-muted text-sm sticky top-0 ${isDark ? "bg-black/30" : "bg-white/30"}  backdrop-blur-sm`}>
            About This Project
          </span>
          {about}
        </p>
